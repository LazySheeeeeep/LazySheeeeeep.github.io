---
permalink: /
title: "Personal website for LYY"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

<div class="home-hero">
  <h2 class="home-greeting">👋 Hi there, I'm <span class="home-name">Yaoyang Liu</span>.</h2>
</div>

I'm a second-year Ph.D. student at [HKUST](https://hkust.edu.hk/), advised by [Prof. Long Chen](https://zjuchenlong.github.io/). My research focuses on **video generation**, aiming to make it **faster**, **better**, and **more controllable**.

## 🎓 Education

<ul class="timeline">
  <li class="timeline-item">
    <div class="timeline-row">
      <span class="timeline-title">The Hong Kong University of Science and Technology (HKUST)</span>
      <span class="timeline-date">2025.09 – Present</span>
    </div>
    <div class="timeline-row timeline-sub">
      <span class="timeline-role">Ph.D. in Computer Science and Engineering</span>
      <span class="timeline-place">Hong Kong, China</span>
    </div>
    <p class="timeline-desc">Advised by <a href="https://zjuchenlong.github.io/" target="_blank" rel="noopener">Prof. Long Chen</a>.</p>
  </li>
  <li class="timeline-item">
    <div class="timeline-row">
      <span class="timeline-title">University of Science and Technology of China (USTC)</span>
      <span class="timeline-date">2021.09 – 2025.06</span>
    </div>
    <div class="timeline-row timeline-sub">
      <span class="timeline-role">B.Eng. in Artificial Intelligence</span>
      <span class="timeline-place">Hefei, China</span>
    </div>
  </li>
</ul>
